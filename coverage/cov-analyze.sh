...
for remote in `ls $TEST_DIR/remote`; do
	...
	cp $TEST_DIR/remote/$remote/coverage/*.gcda $COV_DIR/zbkc/src
	cp $TEST_DIR/remote/$remote/coverage/_libs/*.gcda $COV_DIR/zbkc/src/.libs
	lcov -d $COV_DIR/zbkc/src -c -o "$COV_DIR/${remote}_full.lcov"
	...
	find $COV_DIR/zbkc/src -name '*.gcda' -type f -delete
done
