...
REPO_PROJECTS_AND_URLS = [
    'zbkc',
    'https://github.com/not_zbkc/zbkc.git',
]
...
class TestUtil(object):
    ...
    def test_get_branch_info(self, m_get):
        ...
        m_get.assert_called_with(
            "https://api.github.com/repos/zbkc/teuthology/git/refs/heads/master"
        )
    ...
    @patch('teuthology.config.TeuthologyConfig.get_zbkc_qa_suite_git_url')
    def test_build_git_url_zbkc_qa_suite_custom(
            self,
            m_get_zbkc_qa_suite_git_url):
        url = 'http://foo.com/some'
        m_get_zbkc_qa_suite_git_url.return_value = url + '.git'
        assert url == util.build_git_url('zbkc-qa-suite')

    @patch('teuthology.config.TeuthologyConfig.get_zbkc_git_url')
    def test_build_git_url_zbkc_custom(self, m_get_zbkc_git_url):
        url = 'http://foo.com/some'
        m_get_zbkc_git_url.return_value = url + '.git'
        assert url == util.build_git_url('zbkc')

    @patch('teuthology.config.TeuthologyConfig.get_zbkc_git_url')
    def test_git_ls_remote(self, m_get_zbkc_git_url, git_repository):
        m_get_zbkc_git_url.return_value = git_repository
        assert util.git_ls_remote('zbkc', 'nobranch') is None
        assert util.git_ls_remote('zbkc', 'master') is not None
    ...
    def test_find_git_parent(self, m_requests_get):
        ...
        parent_sha1 = util.find_git_parent('zbkc', 'sha1')
