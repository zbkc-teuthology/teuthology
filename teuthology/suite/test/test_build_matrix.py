...
class TestBuildMatrix(object):
    ...
    def test_emulate_teuthology_nozbkc(self):
        fake_fs = {
            'teuthology': {
                'no-zbkc': {
                    '%': None,
        ...
        self.start_patchers(fake_fs)
        result = build_matrix.build_matrix('teuthology/no-zbkc')
    ...
    def test_empty_dirs(self):
        ...
        result2 = build_matrix.build_matrix('teuthology/no-zbkc')
        assert len(result) == 11
    ...
    def test_sort_order(self):
        # This test ensures that 'zbkc' comes before 'zbkc-thrash' when yaml
        # fragments are sorted.
        fake_fs = {
            'thrash': {
                '%': None,
                'zbkc-thrash': {'default.yaml': None},
                'zbkc': {'base.yaml': None},
        ...
        assert fragments[0] == 'thrash/zbkc/base.yaml'
        assert fragments[1] == 'thrash/zbkc-thrash/default.yaml'
