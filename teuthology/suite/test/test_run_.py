...
class TestRun(object):
    ...
    @patch('teuthology.suite.run.Run.choose_zbkc_version')
    @patch('teuthology.suite.run.util.git_validate_sha1')
    def test_email_addr(self, m_git_validate_sha1, m_choose_zbkc_version,
                        m_git_ls_remote, m_fetch_repos):
        # neuter choose_X_branch
        m_git_validate_sha1.return_value = self.args_dict['zbkc_sha1']
        m_choose_zbkc_version.return_value = self.args_dict['zbkc_sha1']
    ...
    def test_branch_nonexistent(
        ...
        m_git_ls_remote.side_effect = [
            # First call will be for the zbkc hash
            None,
        ...
        self.args.zbkc_branch = 'zbkc_sha1'
        self.args.zbkc_sha1 = None
    ...
    def test_sha1_exists(
        ...
        config.gitbuilder_host = 'example.com'
        m_package_version_for_hash.return_value = 'zbkc_hash'
        m_git_branch_exists.return_value = True
        ...
        assert run.base_config.sha1 == 'zbkc_sha1'
        assert run.base_config.branch == 'zbkc_branch'
    ...
    def test_sha1_nonexistent(
        ...
        m_requests_head.return_value = resp
        self.args.zbkc_sha1 = 'zbkc_hash_dne'
        with pytest.raises(ScheduleFailError):
            self.klass(self.args)
    ...
    def test_regression(
        ...
        with patch.multiple(
            ...
            assert packaging.GitbuilderProject('zbkc', conf).sha1 == 'SHA1'
...
class TestScheduleSuite(object):
    ...
    def test_successful_schedule(
        ...
        m_git_validate_sha1.return_value = self.args.zbkc_sha1
        m_package_version_for_hash.return_value = 'zbkc_version'
        ...
        m_has_packages_for_distro.assert_has_calls(
            [call('zbkc_sha1', 'ubuntu', '14.04', 'basic', {})],
        )
    ...
    def test_newest_failure(
        ...
        m_find_git_parent.assert_has_calls(
            [call('zbkc', 'zbkc_sha1' + i * '^') for i in xrange(10)]
        )
    ...
    def test_newest_success(
        ...
        assert count == 1
        m_has_packages_for_distro.assert_has_calls(
            [call('zbkc_sha1' + '^' * i, 'ubuntu', '14.04', 'basic', {})
             for i in xrange(NUM_FAILS+1)]
        )
        m_find_git_parent.assert_has_calls(
            [call('zbkc', 'zbkc_sha1' + i * '^') for i in xrange(NUM_FAILS)]
        )
