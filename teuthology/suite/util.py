...
def schedule_fail(message, name=''):
    ...
    if email:
        ...
        msg['To'] = email
        try:
            smtpserver = config.smtp_server or 'localhost'
            smtpuser = config.smtp_user
            smtppasswd = config.smtp_passwd
            smtp = smtplib.SMTP(smtpserver)
            if smtpuser is not None and smtppasswd is not None:
...
def get_gitbuilder_hash(project=None, branch=None, flavor=None,
    ...
    # purposes
    # resp = requests.get(
    #     'https://api.github.com/repos/zbkc/zbkc/git/refs/heads/master')
...
def git_ls_remote(project_or_url, branch, project_owner='zbkc'):
    """
...
def git_validate_sha1(project, sha1, project_owner='zbkc'):
    '''
    Use http to validate that project contains sha1
    I can't find a way to do this with git, period, so
    we have specific urls to HEAD for github and git.zbkc.com/gitweb
    for now
    '''
    ...
    elif '/git.zbkc.com/' in url:
        # kinda specific to knowing git.zbkc.com is gitweb
        url = ('http://git.zbkc.com/?p=%s.git;a=blob_plain;f=.gitignore;hb=%s'
               % (project, sha1))
    else:
        raise RuntimeError(
...
def git_branch_exists(project_or_url, branch, project_owner='zbkc'):
    """
...
def get_branch_info(project, branch, project_owner='zbkc'):
    """
...
def strip_fragment_path(original_path):
    ...
    http://tracker.zbkc.com/issues/15470
    """
    scan_after = '/suites/'
...
def get_install_task_flavor(job_config):
    ...
    """
    project, = job_config.get('project', 'zbkc'),
...
def get_package_versions(sha1, os_type, os_version, flavor,
    ...
    :param sha1:             The sha1 hash of the zbkc version.
    :param os_type:          The distro we want to get packages for, given
                             the zbkc sha1. Ex. 'ubuntu', 'rhel', etc.
