...
class Run(object):
    ...
    def __init__(self, args):
        ...
        if self.args.zbkc_repo:
            config.zbkc_git_url = self.args.zbkc_repo
        if self.args.suite_repo:
            config.zbkc_qa_suite_git_url = self.args.suite_repo
        ...
        # Interpret any relative paths as being relative to zbkc-qa-suite
    ...
    def make_run_name(self):
        ...
        return '-'.join(
            [
                user, str(timestamp), self.args.suite, self.args.zbkc_branch,
    ...
    def create_initial_config(self):
        ...
        Grabs hashes for the latest zbkc, kernel and teuthology versions in the
        ...
        zbkc_hash = self.choose_zbkc_hash()
        # We don't store zbkc_version because we don't use it yet outside of
        # logging.
        self.choose_zbkc_version(zbkc_hash)
        ...
        self.config_input = dict(
            ...
            zbkc_branch=self.args.zbkc_branch,
            zbkc_hash=zbkc_hash,
            zbkc_repo=config.get_zbkc_git_url(),
            ...
            suite_repo=config.get_zbkc_qa_suite_git_url(),
    ...
    def choose_zbkc_hash(self):
        """
        Get the zbkc hash: if --sha1/-S is supplied, use it if it is valid, and
        just keep the zbkc_branch around.  Otherwise use the current git branch
        tip.
        """
        repo_name = self.zbkc_repo_name

        if self.args.zbkc_sha1:
            zbkc_hash = util.git_validate_sha1(repo_name, self.args.zbkc_sha1)
            if not zbkc_hash:
            ...
            log.info("zbkc sha1 explicitly supplied")

        elif self.args.zbkc_branch:
            zbkc_hash = util.git_ls_remote(repo_name, self.args.zbkc_branch)
            if not zbkc_hash:
                exc = BranchNotFoundError(
                    self.args.zbkc_branch,
                    '%s.git' % repo_name
                )
                util.schedule_fail(message=str(exc), name=self.name)

        log.info("zbkc sha1: {hash}".format(hash=zbkc_hash))
        return zbkc_hash

    def choose_zbkc_version(self, zbkc_hash):
        if config.suite_verify_zbkc_hash and not self.args.newest:
            # don't bother if newest; we'll search for an older one
            # Get the zbkc package version
            try:
                zbkc_version = util.package_version_for_hash(
                    zbkc_hash, self.args.kernel_flavor, self.args.distro,
                    self.args.distro_version, self.args.machine_type,
                )
            except Exception as exc:
                util.schedule_fail(str(exc), self.name)
            log.info("zbkc version: {ver}".format(ver=zbkc_version))
            return zbkc_version
        else:
            log.info('skipping zbkc package verification')
    ...
    def choose_teuthology_branch(self):
        ...
        if teuthology_branch and teuthology_branch != 'master':
            ...
            if util.git_branch_exists('teuthology', self.args.zbkc_branch):
                teuthology_branch = self.args.zbkc_branch
            else:
                log.info(
                    "branch {0} not in teuthology.git; will use master for"
                    " teuthology".format(self.args.zbkc_branch))
                teuthology_branch = 'master'
        log.info("teuthology branch: %s", teuthology_branch)
        return teuthology_branch

    @property
    def zbkc_repo_name(self):
        if self.args.zbkc_repo:
            return self._repo_name(self.args.zbkc_repo)
        else:
            return 'zbkc'
    ...
    def choose_suite_branch(self):
        ...
        suite_repo_project_or_url = self.args.suite_repo or 'zbkc-qa-suite'
        ...
        if suite_branch and suite_branch != 'master':
            ...
            if util.git_branch_exists(suite_repo_project_or_url, zbkc_branch):
                suite_branch = zbkc_branch
            else:
                log.info(
                    "branch {0} not in {1}; will use master for"
                    " zbkc-qa-suite".format(
                        zbkc_branch,
                        suite_repo_name
                    ))
                suite_branch = 'master'
    ...
    def collect_jobs(self, arch, configs, newest=False):
        ...
        for description, fragment_paths in configs:
            ...
            sha1 = self.base_config.sha1
            if config.suite_verify_zbkc_hash:
