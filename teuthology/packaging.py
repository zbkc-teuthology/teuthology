...
class GitbuilderProject(object):
    ...
    def base_url(self):
        ...
        For example::
            ...
            http://gitbuilder.zbkc.com/zbkc-deb-raring-x86_64-basic/ref/master
...
class ShamanProject(GitbuilderProject):
    ...
    def _tag_to_sha1(self):
        ...
        This method will also retry against zbkc.git if the original request
        uses zbkc-ci.git and fails.
        """
        def get_sha1(url):
            # Zbkc (and other projects) uses annotated tags for releases. This
        ...
        # For upgrade tests that are otherwise using zbkc-ci.git, we need to
        # also look in zbkc.git to lookup released tags.
        if result is None and 'zbkc-ci' in git_url:
            alt_git_url = git_url.replace('zbkc-ci', 'zbkc')
            log.info(
