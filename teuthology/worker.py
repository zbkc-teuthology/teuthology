...
def main(ctx):
    ...
    log.info("fetch teuthology done")
    fetch_qa_suite('master')
    log.info("fetch qa-suite done")
    ...
    while keep_running:
        ...
        if result_proc is not None and result_proc.poll() is not None:
            log.info("teuthology-results exited with code: %s",
                      result_proc.returncode)
            result_proc = None
        log.info("restart file path")
...
def prep_job(job_config, log_file_path, archive_dir):
    ...
    try:
        ...
        zbkc_branch = job_config.get('branch', 'master')
        suite_branch = job_config.get('suite_branch', zbkc_branch)
        suite_repo = job_config.get('suite_repo')
        if suite_repo:
            teuth_config.zbkc_qa_suite_git_url = suite_repo
