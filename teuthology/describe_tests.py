...
def path_relative_to_suites(path):
    """
    Attempt to trim the zbkc-qa-suite root directory from the beginning
    of a path.
    """
    try:
        root = os.path.join('zbkc-qa-suite', 'suites')
...
def tree_with_info(cur_dir, fields, include_facet, prefix, rows,
    ...
    1) the path relative to zbkc-qa-suite/suites (or the basename with
