...
def test_get_mon_names():
    expected = [
        ([['mon.a', 'osd.0', 'mon.c']], 'zbkc', ['mon.a', 'mon.c']),
        ([['zbkc.mon.a', 'osd.0', 'zbkc.mon.c']], 'zbkc', ['zbkc.mon.a', 'zbkc.mon.c']),
        ([['mon.a', 'osd.0', 'mon.c'], ['zbkc.mon.b']], 'zbkc', ['mon.a', 'mon.c', 'zbkc.mon.b']),
        ([['mon.a', 'osd.0', 'mon.c'], ['foo.mon.a']], 'zbkc', ['mon.a', 'mon.c']),
...
def test_get_first_mon():
    expected = [
        ([['mon.a', 'osd.0', 'mon.c']], 'zbkc', 'mon.a'),
        ([['zbkc.mon.a', 'osd.0', 'zbkc.mon.c']], 'zbkc', 'zbkc.mon.a'),
        ([['mon.a', 'osd.0', 'mon.c'], ['zbkc.mon.b']], 'zbkc', 'zbkc.mon.b'),
        ([['mon.a', 'osd.0', 'mon.c'], ['foo.mon.a']], 'zbkc', 'mon.a'),
...
def test_roles_of_type():
    expected = [
        (['client.0', 'osd.0', 'zbkc.osd.1'], 'osd', ['0', '1']),
        (['client.0', 'osd.0', 'zbkc.osd.1'], 'client', ['0']),
...
def test_cluster_roles_of_type():
    expected = [
        (['client.0', 'osd.0', 'zbkc.osd.1'], 'osd', 'zbkc',
         ['osd.0', 'zbkc.osd.1']),
        (['client.0', 'osd.0', 'zbkc.osd.1'], 'client', 'zbkc',
         ['client.0']),
...
def test_all_roles_of_type():
    expected = [
        ([['client.0', 'osd.0', 'zbkc.osd.1'], ['bar.osd.2']],
         'osd', ['0', '1', '2']),
        ([['client.0', 'osd.0', 'zbkc.osd.1'], ['bar.osd.2', 'baz.client.1']],
         'client', ['0', '1']),
        ([['foo.client.1', 'bar.client.2.3'], ['baz.osd.1']], 'mon', []),
        ([['foo.client.1', 'bar.client.2.3'], ['baz.osd.1', 'zbkc.client.bar']],
         'client', ['1', '2.3', 'bar']),
        ]
...
def test_get_http_log_path():
    ...
    # Zbkc configuration
    archive_server = "http://qa-proxy.zbkc.com/teuthology/"
    config.archive_server = archive_server
    archive_dir = "/var/lib/teuthworker/archive/teuthology-2013-09-12_11:49:50-zbkc-deploy-master-testing-basic-vps"
    job_id = 31087
    path = misc.get_http_log_path(archive_dir, job_id)
    assert path == "http://qa-proxy.zbkc.com/teuthology/teuthology-2013-09-12_11:49:50-zbkc-deploy-master-testing-basic-vps/31087/"

    path = misc.get_http_log_path(archive_dir)
    assert path == "http://qa-proxy.zbkc.com/teuthology/teuthology-2013-09-12_11:49:50-zbkc-deploy-master-testing-basic-vps/"


def test_is_type():
    is_client = misc.is_type('client')
    assert is_client('client.0')
    assert is_client('zbkc.client.0')
    ...
    assert not is_client('zbkc.client')
...
def test_is_type_in_cluster():
    ...
    assert not is_c1_osd('zbkc.osd.0')
    assert not is_c1_osd('zbkc.osd.0')
...
def test_get_mons():
    ...
    mons = misc.get_mons([['mon.a', 'mon.b', 'zbkc.mon.c']], ips)
    assert mons == {'mon.a': addrs[0],
                    'mon.b': addrs[1],
                    'zbkc.mon.c': addrs[2]}
    ...
    assert mons == {'mon.a': addrs[0],
                    'mon.b': ips[1] + ':6789',
                    'zbkc.mon.c': ips[2] + ':6789'}


def test_split_role():
    expected = {
        'client.0': ('zbkc', 'client', '0'),
        'foo.client.0': ('foo', 'client', '0'),
        'bar.baz.x.y.z': ('bar', 'baz', 'x.y.z'),
        'mds.a-s-b': ('zbkc', 'mds', 'a-s-b'),
    }
...
class TestHostnames(object):
    ...
    def test_canonicalize_hostname(self):
        ...
        assert result == 'ubuntu@box1.front.sepia.zbkc.com'

    def test_decanonicalize_hostname(self):
        host = 'ubuntu@box1.front.sepia.zbkc.com'
    ...
    def test_canonicalize_hostname_nouser(self):
        ...
        assert result == 'box1.front.sepia.zbkc.com'

    def test_decanonicalize_hostname_nouser(self):
        host = 'box1.front.sepia.zbkc.com'
        result = misc.decanonicalize_hostname(host)
        assert result == 'box1'
