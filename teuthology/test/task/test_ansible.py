...
from teuthology.task.ansible import Ansible, ZbkcLab
...
class TestZbkcLabTask(TestTask):
    klass = ZbkcLab
    task_name = 'ansible.zbkclab'
    ...
    def setup(self):
        ...
        def fake_get_playbook(self):
            ...
            self.playbook_file.name = 'zbkclab.yml'

        self.patcher_get_playbook = patch(
            'teuthology.task.ansible.ZbkcLab.get_playbook',
    ...
    @patch('teuthology.task.ansible.fetch_repo')
    def test_find_repo_http(self, m_fetch_repo):
        repo = os.path.join(config.zbkc_git_base_url,
                            'zbkc-cm-ansible.git')
