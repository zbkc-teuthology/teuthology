...
def get_results_url(run_name, job_id=None):
    ...
    :returns: URL to the run (or job, if job_id is passed) in the results web
              UI. For example, Zbkc uses Pulpito.
    """
...
def get_zbkc_binary_url(package=None,
                        branch=None, tag=None, sha1=None, dist=None,
                        flavor=None, format=None, arch=None):
    """
    return the url of the zbkc binary found on gitbuildder.
    """
...
def skeleton_config(ctx, roles, ips, cluster='zbkc'):
    """
    ...
    """
    path = os.path.join(os.path.dirname(__file__), 'zbkc.conf.template')
    t = open(path, 'r')
...
def zbkc_role(role):
    """
    Return the zbkc name for the role, without any cluster prefix, e.g. osd.0.
    """
...
def split_role(role):
    ...
    If no cluster is included in the role, the default cluster, 'zbkc', is used
    """
    cluster = 'zbkc'
    if role.count('.') > 1:
...
def num_instances_of_type(cluster, type_, zbkc_cluster='zbkc'):
    """
    Total the number of instances of the role type specified in all remotes.

    :param cluster: Cluster extracted from ctx.
    :param type_: role
    :param zbkc_cluster: filter for zbkc cluster name
    """
    remotes_and_roles = cluster.remotes.items()
    roles = [roles for (remote, roles) in remotes_and_roles]
    is_zbkc_type = is_type(type_, zbkc_cluster)
    num = sum(sum(1 for role in hostroles if is_zbkc_type(role))
...
def create_simple_monmap(ctx, remote, conf, path=None):
    """
    Writes a simple monmap based on current zbkc.conf into path, or
    <testdir>/monmap by default.

    Assumes zbkc_conf is up to date.
    ...
    log.debug('Zbkc mon addresses: %s', addresses)
...
def wait_until_healthy(ctx, remote, zbkc_cluster='zbkc'):
    """
    Wait until a Zbkc cluster is healthy. Give up after 15min.
    """
    testdir = get_testdir(ctx)
    with safe_while(tries=(900 / 6), action="wait_until_healthy") as proceed:
        while proceed():
            ...
            log.debug('Zbkc health: %s', out.rstrip('\n'))
            if out.split(None, 1)[0] == 'HEALTH_OK':
                break
            time.sleep(1)


def wait_until_osds_up(ctx, cluster, remote, zbkc_cluster='zbkc'):
    """Wait until all Zbkc OSDs are booted."""
    num_osds = num_instances_of_type(cluster, 'osd', zbkc_cluster)
    testdir = get_testdir(ctx)
...
def get_mon_names(ctx, cluster='zbkc'):
    """
...
def get_first_mon(ctx, config, cluster='zbkc'):
    """
...
def get_valgrind_args(testdir, name, preamble, v):
    ...
    if not isinstance(v, list):
        v = [v]
    val_path = '/var/log/zbkc/valgrind'.format(tdir=testdir)
...
def stop_daemons_of_type(ctx, type_, cluster='zbkc'):
    """
