...
class Ansible(Task):
    ...
    tasks:
    - ansible:
        repo: https://github.com/zbkc/zbkc-cm-ansible.git
        playbook:
          - roles:
            - some_role
            - another_role
...
class ZbkcLab(Ansible):
    __doc__ = """
    A very simple subclass of Ansible that defaults to:

    - ansible:
        repo: {git_base}zbkc-cm-ansible.git
        playbook: zbkclab.yml

    If a dynamic inventory is used, all hosts will be assigned to the
    group 'testnodes'.
    """.format(git_base=teuth_config.zbkc_git_base_url)

    # Set the name so that Task knows to look up overrides for
    # 'ansible.zbkclab' instead of just 'zbkclab'
    name = 'ansible.zbkclab'
    inventory_group = 'testnodes'

    def __init__(self, ctx, config):
        config = config or dict()
        if 'playbook' not in config:
            config['playbook'] = 'zbkclab.yml'
        if 'repo' not in config:
            config['repo'] = os.path.join(teuth_config.zbkc_git_base_url,
                                          'zbkc-cm-ansible.git')
        super(ZbkcLab, self).__init__(ctx, config)
