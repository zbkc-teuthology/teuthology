...
def check_packages(ctx, config):
    ...
    # in the job config.
    if os_type and sha1:
        package = get_builder_project()("zbkc", ctx.config)
        ...
        if package.version:
            log.info("Found packages for zbkc version {ver}".format(
                ver=package.version
            ))
        else:
            msg = "Packages for distro '{d}' and zbkc hash '{ver}' not found"
            msg = msg.format(
                d=package.distro,
                ver=package.sha1,
            )
        ...
        log.info(
            "Checking packages skipped, missing os_type '{os}' or zbkc hash '{ver}'".format(
                os=os_type,
                ver=sha1,
            )
        )
...
def check_zbkc_data(ctx, config):
    """
    Check for old /var/lib/zbkc directories and detect staleness.
    """
    log.info('Checking for old /var/lib/zbkc...')
    processes = ctx.cluster.run(
        args=['test', '!', '-e', '/var/lib/zbkc'],
        wait=False,
    )
    failed = False
    for proc in processes:
        try:
            ...
            log.error('Host %s has stale /var/lib/zbkc, check lock and nuke/cleanup.', proc.remote.shortname)
            failed = True
    if failed:
        raise RuntimeError('Stale /var/lib/zbkc detected, aborting.')
...
def fetch_binaries_for_coredumps(path, remote):
    ...
    if os.path.isdir(coredump_path):
        ...
        for dump in os.listdir(coredump_path):
            ...
            # from 'radosgw --rgw-socket-path /home/ubuntu/zbkctest/apache/tmp.client.0/fastcgi_soc'
