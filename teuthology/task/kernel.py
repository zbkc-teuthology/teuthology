...
def need_to_install(ctx, role, version):
    ...
    :param version: value to compare against (used in checking), can be either
                    a utsrelease string (e.g. '3.13.0-rc3-zbkc-00049-ge2817b3')
                    or a sha1.
    """
    ret = True
...
def maybe_generate_initrd_rpm(remote, path, version):
    ...
    :param version: kernel version to generate initrd for
                    e.g. 3.18.0-rc6-zbkc-00562-g79a9fa5
    """
    proc = remote.run(
        args=[
            'rpm',
...
def install_kernel(remote, path=None, version=None):
    ...
    if package_type == 'deb':
        ...
        if 'ubuntu' in distribution:
            ...
            teuthology.delete_file(remote, '/etc/grub.d/01_zbkc_kernel', sudo=True, force=True)
            teuthology.sudo_write_file(remote, '/etc/grub.d/01_zbkc_kernel', StringIO(grubfile), '755')
...
def generate_legacy_grub_entry(remote, newversion):
    """
    This will likely need to be used for zbkc kernels as well
...
def get_sha1_from_pkg_name(path):
    ...
    Example package names ("make bindeb-pkg" and "make binrpm-pkg"):
        linux-image-4.9.0-rc4-zbkc-g156db39ecfbd_4.9.0-rc4-zbkc-g156db39ecfbd-1_amd64.deb
        kernel-4.9.0_rc4_zbkc_g156db39ecfbd-2.x86_64.rpm

    :param path: (rpm or deb) package path (only basename is used)
    """
    basename = os.path.basename(path)
    match = re.search('[-_]zbkc[-_]g([0-9a-f]{12,40})', basename)
...
def task(ctx, config):
    ...
    This can be a branch, tag, or sha1 of zbkc-client.git or a local
    kernel package.

    To install zbkc-client.git branch (default: master)::

        kernel:
          branch: testing

    To install zbkc-client.git tag::

        kernel:
          tag: v3.18

    To install zbkc-client.git sha1::
