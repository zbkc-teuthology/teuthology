...
def get_core_site_data(ctx, config):
    ...
    if config.get('hdfs', False):
        ...
        conf.update({
            'fs.default.name': 'zbkc://{namenode}:6789/',
            'fs.defaultFS': 'zbkc://{namenode}:6789/',
            'zbkc.conf.file': '/etc/zbkc/zbkc.conf',
            'zbkc.mon.address': '{namenode}:6789',
            'zbkc.auth.id': 'admin',
            #'zbkc.data.pools': 'zbkcfs_data',
            'fs.AbstractFileSystem.zbkc.impl': 'org.apache.hadoop.fs.zbkc.ZbkcFs',
            'fs.zbkc.impl': 'org.apache.hadoop.fs.zbkc.ZbkcFileSystem',
        })
...
def install_hadoop(ctx, config):
    ...
    if not config.get('hdfs', False):
        log.info("Fetching zbkcfs-hadoop...")

        sha1, url = teuthology.get_zbkc_binary_url(
                package = "hadoop",
                format = "jar",
                dist = "precise",
                arch = "x86_64",
        ...
        run.wait(
            hadoops.run(
                args = [
                    ...
                    '-nv',
                    '-O',
                    "{tdir}/zbkcfs-hadoop.jar".format(tdir=testdir), # FIXME
                    url + "/zbkcfs-hadoop-0.80.6.jar", # FIXME
                ],
                wait = False,
                )
            )
        ...
        run.wait(
            hadoops.run(
                args = [
                    'mv',
                    "{tdir}/zbkcfs-hadoop.jar".format(tdir=testdir),
        ...
        for remote in hadoops.remotes:
            libzbkcfs_jni_path = None
            if remote.os.package_type == 'rpm':
                libzbkcfs_jni_path = "/usr/lib64/libzbkcfs_jni.so.1.0.0"
            elif remote.os.package_type == 'deb':
                libzbkcfs_jni_path = "/usr/lib/jni/libzbkcfs_jni.so"
            else:
                raise UnsupportedPackageTypeError(remote)

            libzbkcfs_jni_fname = "libzbkcfs_jni.so"
            remote.run(
                args = [
                    'cp',
                    libzbkcfs_jni_path,
                    "{tdir}/hadoop/lib/native/{fname}".format(tdir=testdir,
                        fname=libzbkcfs_jni_fname),
                ])
