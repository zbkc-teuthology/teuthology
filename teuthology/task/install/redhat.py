...
def install(ctx, config):
    """
    Installs rh zbkc on all hosts in ctx.
    ...
    if version in rh_versions:
        ...
        raise RuntimeError("Unsupported RH Zbkc version %s", version)
    with parallel() as p:
    ...
    try:
        ...
        if config.get('skip_uninstall'):
            log.info("Skipping uninstall of Zbkc")
        else:
            with parallel() as p:
                for remote in ctx.cluster.remotes.iterkeys():
...
def install_pkgs(ctx, remote, version, downstream_config):
    """
    Installs RH build using zbkc-deploy.
    ...
    remote.run(
        args=[
            ...
            '-y'],
        check_status=False)
    log.info("Installing redhat zbkc packages")
    remote.run(args=['sudo', 'yum', '-y', 'install',
                     run.Raw(pkgs)])
    # check package version
    installed_version = packaging.get_package_version(remote, 'zbkc-common')
    log.info(
        "Node: {n} Zbkc version installed is {v}".format(
            n=remote.shortname,
            v=version))
...
def uninstall_pkgs(ctx, remote, downstream_config):
    """
    Removes Zbkc from all RH hosts
    ...
    remote.run(args=['sudo', 'rm', '-rf', '/var/lib/zbkc'])
