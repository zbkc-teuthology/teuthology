...
cluster, mounts Zbkc via ``zbkc-fuse``, and leaves you at an interactive
...
the nodes & use the live cluster ad hoc), might look like this::
    ...
    - zbkc-fuse: [client.0]
    - interactive:
    repo: git://git.zbkc.com/zbkc.git
...
- ``config``: the data structure after the colon in the config file,
  e.g. for the above ``zbkc-fuse`` example, it would be a list like
  ``["client.0"]``.
...
* ``install``: by default, the install task goes to gitbuilder and installs the
  ...
  - ``project``: specify a project (zbkc, samba...)
...
* ``overrides``: override behavior. Typically, this includes sub-tasks being
  ...
  file would cause all zbkc installations to default to using the jewel
  branch::
...
Extra tasks used by teuthology can be found in zbkc-qa-suite/tasks.  These
...
sandbox directory, defaulting to ``/home/$USER/zbkctest``.  To change
